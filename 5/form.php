...
<div>
		       ...
		       <input name ="email" <?php  if(!empty($_COOKIE['err_email'])||!empty($_COOKIE['err_email2'])) {print 'class="err"';} ?> 
					   placeholder="ваша почта"value="<?php if(!empty($_SESSION["email"])) {echo $_SESSION["email"];}?>" id="email1"/>
							 </label><div >	<?php if(!empty($_COOKIE['err_email'])) {echo "Заполните почту."; setcookie("err_email","");} else{echo '';}if(!empty($_SESSION["email"])) {echo $_SESSION["email"];}?></div><div >	<?php if(!empty($_COOKIE['err_email2'])) {echo "отсутствует символ '@'"; setcookie("err_email2","");} else{echo '';}?></div>
							</div>
							 <div>
